	...
	builderCtrl := builder.ControllerManagedBy(userMgr).
		...
		WatchesRawSource(source.Kind(seedMgr.GetCache(), &kubermaticv1.PolicyBinding{},
			...
			predicate.Or(
				...
				predicate.TypedFuncs[*kubermaticv1.PolicyBinding]{
					UpdateFunc: func(e event.TypedUpdateEvent[*kubermaticv1.PolicyBinding]) bool {
...
func (r *reconciler) reconcile(ctx context.Context, log *zap.SugaredLogger, binding *kubermaticv1.PolicyBinding, cluster *kubermaticv1.Cluster) error {
	...
	if err := kuberneteshelper.TryAddFinalizer(ctx, r.seedClient, binding, kubermaticv1.PolicyBindingCleanupFinalizer); err != nil {
...
func (r *reconciler) handlePolicyBindingCleanup(ctx context.Context, binding *kubermaticv1.PolicyBinding) error {
	...
	if kuberneteshelper.HasFinalizer(binding, kubermaticv1.PolicyBindingCleanupFinalizer) {
		...
		if err := kuberneteshelper.TryRemoveFinalizer(ctx, r.seedClient, binding, kubermaticv1.PolicyBindingCleanupFinalizer); err != nil {
